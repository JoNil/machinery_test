...
static void engine_update__cave_component(tm_engine_o *inst, tm_engine_update_set_t *data)
{
    ...
    {
        struct tm_cave_component_t *cave_component = a->components[0];
        tm_transform_component_t *transform = a->components[1];
        tm_link_component_t *link = a->components[2];

        for (uint32_t i = 0; i < a->n; ++i)
        {
            if (!cave_component[i].y0)
                cave_component[i].y0 = transform[i].tm.pos.y;
            const float y = cave_component[i].y0 + cave_component[i].amplitude * sinf((float)t * cave_component[i].frequency);
...
static void component__register_engine(struct tm_entity_context_o *ctx)
{
    ...
    const tm_engine_i cave_component_engine = {
        .name = "Cave Component",
        .num_components = 3,
        .components = {cave_component, transform_component, link_component},
